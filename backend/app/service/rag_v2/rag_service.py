...
from app.schema.IntelligentRecommendation_schemas import IntelligentRecommendationRequest, RetrievalRequest, \
    RerankingStrategy, IntelligentRecommendationResponse, SearchStrategy
from app.service.rag_v2.rerank_service import RerankService
from app.service.rag_v2.retrieval_serivce import RetrievalService
...
class RagService:
      ...
      async def generate_intelligent_recommendation(
          ...
          try:
              ...
              # ========== 2. 根据策略执行重排序 ==========
              best_recommendations = await self.rerank_service.llm_rank_all_scenarios(
      ...
      async  def generate_simple_recommendation(self,
          ...
          try:
              ...
              # ========== 2. 根据策略执行重排序 ==========
              best_recommendations = await self.rerank_service.simple_rank_all_scenarios(
                  all_scenarios=final_scenarios,
                  patient_info=request.patient_info,
                  clinical_context=request.clinical_context,
                  strategy=strategy,
      ...
      async def generate_fincal_recommendation(self,
                                               request: IntelligentRecommendationRequest,
                                               medical_dict: Dict[str, Any]
                                               ):
          """
          ...
          try:
              ...
              # ========== 2. 根据策略执行重排序 ==========
              best_recommendations = await self.rerank_service.llm_rerank_final_choices(
                  all_scenarios=final_scenarios,
                  patient_info=request.patient_info,
                  clinical_context=request.clinical_context,
                  strategy=strategy,
      ...
      async def generate_simple_fincal_recommendation(self,request: IntelligentRecommendationRequest,
                                               medical_dict: Dict[str, Any]):
         standard_query = ""
         start_time = time.time()
         try:
             ...
             # ========== 2. 根据策略执行重排序 ==========
             best_recommendations = await self.rerank_service.simple_llm_rank_final_choices(
                 all_scenarios=final_scenarios,
                 patient_info=request.patient_info,
                 clinical_context=request.clinical_context,
                 strategy=strategy,
