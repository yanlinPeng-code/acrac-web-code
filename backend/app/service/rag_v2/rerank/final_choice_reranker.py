...
class FinalChoiceReranker(BaseReranker):


      def __init__(self,
                   adaptive_service:FinalChoiceAdaptiveReranker
                   ):
          super().__init__()
      ...
      async def _handle_llm_recommendation_only_strategy(self,
          ...
          # 使用自适应引擎进行LLM推荐项目重排序
          recommendations = await self.adaptive_service.get_recommendations(
      ...
      async def _handle_llm_scenario_and_recommendation_strategy(self,
          ...
          token_nums = len(self.prompt_builder.tokenizer.encode(prompt))
          threshold = settings.OLLAMA_LLM_MAX_TOKENS
          ...
          if token_nums < threshold - 1500:
              ...
              # 第二步：LLM推荐项目重排序
              recommendations = await self.adaptive_service.get_recommendations(
                  confirmed_scenarios=filtered_scenarios_with_recommends,
                  patient_info=patient_info,
                  clinical_context=clinical_context,
                  max_recommendations_per_scenario=max_recommendations_per_scenario,
                  direct_return=direct_return, use_adaptive=True
