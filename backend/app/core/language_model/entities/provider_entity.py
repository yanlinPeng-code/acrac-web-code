...
from typing_extensions import Union, Type, Optional, Any
from fastapi import HTTPException

from app.utils.helper.helper import dynamic_import
...
class Provider(BaseModel):
    ...
    model_class_map: dict[str,Type[Any]] = Field(default_factory=dict)  # 模型类映射
    
    model_config = {
        "arbitrary_types_allowed": True,  # 允许任意类型，包括 Type[Any]
    }

    @model_validator(mode="after")
    def validate_provider( self: "Provider") -> "Provider":
        ...
        for model_type in provider_entity.supported_model_types:
            ...
            self.model_class_map[model_type] = dynamic_import(
                f"app.core.language_model.providers.{provider_entity.name}.{model_type.value}",
                symbol_name
            )
        ...
        if not isinstance(positions_yaml_data, list):
            raise HTTPException(status_code=500, detail="positions.yaml数据格式错误")
    ...
    def get_model_class(self, model_type: ModelType) -> Optional[Type[Any]]:
        """根据传递的模型类型获取该提供者的模型类"""
        model_class = self.model_class_map.get(model_type, None)
        if model_class is None:
            raise HTTPException(status_code=404, detail=f"该模型类不存在，请核实后重试")
        return model_class
    ...
    def get_model_entity(self, model_name: str) -> Optional[ModelEntity]:
        ...
        if model_entity is None:
            raise HTTPException(status_code=404, detail=f"该模型实体不存在，请核实后重试")
        return model_entity
